...
```text
After changing customer-facing behavior, decide whether browser verification would be useful.
...
If this repository has `possum.config.json`, `possum audit` remains available as a compatibility alias after the implementation builds or the app can start.

Inspect the generated `.possum/runs/<runId>/report.md`. For each relevant confirmed finding, inspect the finding artifacts and generated repro, fix the app, then run `possum replay <reproPath>` or another Possum verification to verify the customer failure no longer reproduces.

If `possum.config.json` is missing, suggest running `possum init` instead of guessing the app URL or startup command.
```

## CLI Workflow

After Codex finishes a user-facing code change:

```bash
possum verify-diff
```

Use explicit feature verification when acceptance criteria are known:

```bash
possum verify-feature --brief feature.json
```

Use broad app verification when you need general confidence:
...
1. Call `verify_diff` after user-facing code changes, `verify_feature` when explicit acceptance criteria exist, or `verify_app` for broader app confidence.
